...
def get_vehiculo_by_id(db: Session, au_id: int):
    return db.query(Models.model_vehiculo.Vehiculo).filter(Models.model_vehiculo.Vehiculo.au_id == au_id).first()
...
def update_vehiculo(db: Session, au_id: int, vehiculo: Schemas.schema_vehiculo.VehiculoCreate):
    db_vehiculo = db.query(Models.model_vehiculo.Vehiculo).filter(Models.model_vehiculo.Vehiculo.au_id == au_id).first()
    ...
    db_vehiculo.au_estado = vehiculo.au_estado
    db_vehiculo.fecha_modificacion = vehiculo.fecha_modificacion
    db.commit()
    db.refresh(db_vehiculo)
    return db_vehiculo
...
def get_vehiculo_by_marca(db: Session, au_marca: str):
    return db.query(Models.model_vehiculo.Vehiculo).filter(Models.model_vehiculo.Vehiculo.au_marca == au_marca).first()

def get_vehiculo_by_modelo(db: Session, au_modelo: str):
    return db.query(Models.model_vehiculo.Vehiculo).filter(Models.model_vehiculo.Vehiculo.au_modelo == au_modelo).first()

def get_vehiculo_by_anio(db: Session, au_anio: int):
    return db.query(Models.model_vehiculo.Vehiculo).filter(Models.model_vehiculo.Vehiculo.au_anio == au_anio).first()

def get_vehiculo_by_color(db: Session, au_color: str):
    return db.query(Models.model_vehiculo.Vehiculo).filter(Models.model_vehiculo.Vehiculo.au_color == au_color).first()
