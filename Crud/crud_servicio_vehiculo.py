...
from Models.model_servicio_vehiculo import Solicitud
...
def create_servicio_vehiculo(db: Session, servicio_vehiculo: Schemas.schema_servicio_vehiculo.ServicioVehiculoCreate):
    db_servicio_vehiculo = ServicioVehiculo(
...
def get_servicio_vehiculo_by_au_id(db: Session, au_id: int):
    return db.query(ServicioVehiculo).filter(ServicioVehiculo.au_id == au_id).first()

def get_servicio_vehiculo_by_cajero_id(db: Session, cajero_id: int):
    return db.query(ServicioVehiculo).filter(ServicioVehiculo.cajero_id == cajero_id).first()
