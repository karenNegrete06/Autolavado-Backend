...
def create_servicio_vehiculo(servicio_vehiculo: Schemas.schema_servicio_vehiculo.ServicioVehiculoCreate, db: Session = Depends(get_db)):
    db_servicio_vehiculo = Crud.crud_servicio_vehiculo.get_servicio_vehiculo_by_nombre(db, fecha=servicio_vehiculo.fecha, hora = servicio_vehiculo.hora)
    if db_servicio_vehiculo:
        raise HTTPException(status_code=400, detail="Servicio Vehiculo existente intenta nuevamente")
    return Crud.crud_servicio_vehiculo.create_servicio_vehiculo(db=db, servicio_vehiculo=servicio_vehiculo)
